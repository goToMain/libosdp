...
int test_osdp_commands_setup(struct test *t)
{
	...
	osdp_pd_set_command_callback(priv.pd_ctx, test_check_command, &priv);
...
struct test_osdp_commands {
	...
	{ CMD_POLL, REPLY_ACK },       { CMD_ID, REPLY_PDID },
	{ CMD_CAP, REPLY_PDCAP },      { CMD_LSTAT, REPLY_LSTATR },
	{ CMD_RSTAT, REPLY_RSTATR },   { CMD_OUT, REPLY_ACK },
	{ CMD_LED, REPLY_ACK },	       { CMD_BUZ, REPLY_ACK },
	{ CMD_TEXT, REPLY_ACK },       { CMD_CHLNG, REPLY_CCRYPT },
	{ CMD_SCRYPT, REPLY_RMAC_I },  { CMD_MFG, REPLY_ACK },
	{ CMD_COMSET, REPLY_COM },     { CMD_ACURXSIZE, REPLY_ACK },
	{ CMD_KEEPACTIVE, REPLY_ACK }, { CMD_ABORT, REPLY_ACK },
};

#define STRINGIFY(x) #x
#define TOSTRING(x)  STRINGIFY(x)
#define __ASSERT(cond, str)                                                    \
	if (!(cond)) {                                                         \
		printf(SUB_2 "Assert %s failed!\n", str);                      \
		return -1;                                                     \
	}
...
void run_osdp_commands_tests(struct test *t)
{
	...
	for (i = 0; i < asize; i++) {
		...
		if (pd_pd->reply_id != c->reply) {
			TEST_REPORT(t, false);
			printf(SUB_2
			       "Invalid Reply(%x) for Cmd(%d). Expected Reply(%x)",
